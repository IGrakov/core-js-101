...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (top1 < top2 || left1 < left2) return top2 < top1 + h1 && left2 < left1 + w1;
  return top1 < top2 + h2 && left1 < left2 + w2;
...
function isInsideCircle(circle, point) {
  const x2 = (point.x - circle.center.x) ** 2;
  const y2 = (point.y - circle.center.y) ** 2;
...
function isCreditCardNumber(ccn) {
  const number = ccn.toString().split('').map((el) => parseInt(el, 10));
  for (let i = number.length - 2; i >= 0; i -= 2) {
    number[i] *= 2;
    if (number[i] > 9) number[i] -= 9;
  }
  const res = number.reduce((sum, el) => sum + el);
  return res % 10 === 0;
}
...
function getDigitalRoot(num) {
  let sum;
  let numAsStr = num.toString().split('');
  while (numAsStr.length > 1) {
    sum = 0;
    for (let i = 0; i < numAsStr.length; i += 1) {
      sum += parseInt(numAsStr[i], 10);
    }
    numAsStr = sum.toString().split('');
  }
  return parseInt(numAsStr[0], 10);
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (closingBrackets.includes(str[i])) {
      if (stack[stack.length - 1] === openingBrackets[closingBrackets.indexOf(str[i])]) {
        stack.pop();
      } else {
        return false;
      }
